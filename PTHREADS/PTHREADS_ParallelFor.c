...
void *parallel(void *param){
	data *dados = (data *)param;
	
	int i, k, ite=600,progress=0;
	double x, y;
	double dt=0.001;
	int size=round(4.0/dt);

	
	int start = dados->tid * size/N_THREADS;
	int final = (dados->tid+1) *size/N_THREADS; 
	printf("start: %d, final: %d, %d\n", start, final, dados->tid);
	
	//for(i = start; i < final; i++){ //Blocos
	for(i=dados->tid;i<size;i+=N_THREADS){ //Em seguida
        x=-2.0+((double)i*dt);
...
int main(void){
    ...
    int i,j,k, t;   
    double x,y;
    data *dados = (data *)malloc(N_THREADS * sizeof(data));
    for (t=0; t<N_THREADS; t++){
    	dados[t].tid = t;
      	pthread_create(&threads[t], NULL, parallel, &dados[t]);
    }
